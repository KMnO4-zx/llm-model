...
# Fallback implementations for missing kernel functions
def act_quant(x: torch.Tensor, block_size: int, scale_fmt: Optional[str] = None) -> Tuple[torch.Tensor, torch.Tensor]:
    """Fallback implementation of act_quant for non-FP8 environments"""
    # Simple quantization simulation - just return x and dummy scale
...
def fp8_gemm(x: torch.Tensor, x_scale: torch.Tensor, weight: torch.Tensor, weight_scale: torch.Tensor) -> torch.Tensor:
    """Fallback implementation of fp8_gemm for non-FP8 environments"""
    # Just use standard linear operation as fallback
    return torch.nn.functional.linear(x, weight)

def fp8_index(q: torch.Tensor, weights: torch.Tensor, k_cache: torch.Tensor, k_scale_cache: torch.Tensor) -> torch.Tensor:
    """Fallback implementation of fp8_index for non-FP8 environments"""
    # Simple attention scoring as fallback
    bsz, seqlen, head_dim = q.shape
    cache_len = k_cache.shape[1]

    # Reshape for batch matrix multiplication
    q = q.view(bsz, seqlen, head_dim)
    k_cache = k_cache.view(bsz, cache_len, head_dim)
...
def linear(x: torch.Tensor, weight: torch.Tensor, bias: Optional[torch.Tensor] = None,
    ...
    Notes:
        - For MPS compatibility, we use standard linear operations without FP8 quantization.
    """
    assert bias is None

    # For MPS compatibility, always use standard linear operation
...
class Linear(nn.Module):
    ...
    Args:
        ...
        dtype (optional): Data type for the layer. Defaults to `torch.float32` for MPS.
    """
    dtype = torch.float32  # Changed from bfloat16 for MPS compatibility
    scale_fmt: Optional[str] = None
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = False, dtype = None):
        ...
        # Initialize weights properly for MPS compatibility
        self.weight = nn.Parameter(torch.empty(out_features, in_features, dtype=actual_dtype))
        nn.init.normal_(self.weight, std=0.02)

        # Simplified initialization for MPS compatibility
        self.register_parameter("scale", None)

        if bias:
            self.bias = nn.Parameter(torch.empty(out_features, dtype=actual_dtype))
...
def rotate_activation(x: torch.Tensor) -> torch.Tensor:
    # Support both bf16 and fp32 for MPS compatibility
    ...
    try:
        ...
        # Fallback for non-CUDA environments: apply a simple rotation
        hidden_size = x.size(-1)
        # Simple orthogonal transformation as fallback
        rotation_matrix = torch.randn(hidden_size, hidden_size, dtype=x.dtype, device=x.device)
        q, _ = torch.linalg.qr(rotation_matrix)
        return torch.nn.functional.linear(x, q) * (hidden_size ** -0.5)
...
class Indexer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, qr: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        # Store regular tensors instead of quantized ones for MPS compatibility
        self.k_cache[:bsz, start_pos:end_pos] = k
        self.k_scale_cache[:bsz, start_pos:end_pos] = torch.ones(bsz, seqlen, self.head_dim // block_size, dtype=torch.float32, device=k.device)
        weights = self.weights_proj(x) * self.n_heads ** -0.5
        # Use regular tensors instead of quantized ones for MPS compatibility
        dummy_scale = torch.ones(1, dtype=torch.float32, device=q.device)
        index_score = fp8_index(q.contiguous(), weights.unsqueeze(-1), self.k_cache[:bsz, :end_pos].contiguous(), self.k_scale_cache[:bsz, :end_pos].contiguous())
        if mask is not None:
            index_score += mask
...
if __name__ == "__main__":
    torch.set_default_dtype(torch.float32)  # MPS supports float32
    torch.set_default_device("mps")
    torch.manual_seed(0)
    args = ModelArgs()
    args.dtype = "bf16"  # Keep dtype as bf16 for model config, but use float32 for tensors
